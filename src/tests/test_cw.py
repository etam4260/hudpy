...
def test_cw_all_types():
    ...
    zip_tract = huduser.hud_cw(type = 1, query = "35213",
                      year = ["2010", "2011"], quarter = ["1"])

    assert len(zip_tract) >= 1


    zip_county = huduser.hud_cw(type = "2", query = "35213",
                       year = ["2016", "2020"], quarter = ["2"])

    assert len(zip_county) >= 1


    zip_cbsa = huduser.hud_cw(type = 3, query = 35213,
                     year = ["2012", "2011"], quarter = ["3"])

    assert len(zip_cbsa) >= 1


    zip_cbsadiv = huduser.hud_cw(type = 4, query = "22031",
                        year = ["2017", "2019"], quarter = ["4"])

    assert len(zip_cbsadiv) >= 1


    zip_cd = huduser.hud_cw(type = "5", query = "35213",
                   year = [2011, "2012"], quarter = ["1", "2"])

    assert len(zip_cd) >= 1


    tract_zip = huduser.hud_cw(type = 6, query = "48201223100",
                      year = ["2017", "2010"],
                      quarter = ["1", "2", "3"])
    ...
    county_zip = huduser.hud_cw(type = 7, query = "22031",
                       year = ["2010", "2011"],
                       quarter = ["1", "2", "3", "4"])
    ...
    county_zip = huduser.hud_cw(type = 7,
                       query = list(map(lambda x: x[0:5], hudmisc.hud_state_counties("md")["fips_code"])),
                       year = ["2010"], quarter = ["1"])
    ...
    cbsa_zip = huduser.hud_cw(type = 8, query = "10140",
    ...
    # metropolitan areas.
    cbsadiv_zip = huduser.hud_cw(type = 9, query = "10380",
                        year = ["2017"], quarter = ["4"])

    assert len(cbsadiv_zip) >= 1


    cd_zip = huduser.hud_cw(type = 10, query = "2202",
                   year = ["2010", "2011"], quarter = ["4", "3"])

    assert len(cd_zip) >= 1

    # Testing ZIP CODE _> COUNTYSUB
    zip_countysub = huduser.hud_cw(type = 11, query = "35213",
    ...
    countysub_zip = huduser.hud_cw(type = 12, query = "4606720300 ",
                          year = ["2019", "2019", "2019"],
                          quarter = ["4", "4"])
...
def test_cw_diff_years():
    ...
    with pytest.raises(ValueError) as e_info:
        huduser.hud_cw(type = 7, query = "22031",
                         year = ["2010", "2011", "2024"],
                         quarter = ["1", "2", "3", "4"])
...
def test_cw_wrong_query():
    with pytest.raises(ValueError) as e_info:
        huduser.hud_cw(type = 7, query = "22031241231",
                      year = ["2010", "2011"],
                      quarter = ["1", "2", "3", "4"])

  # No zip code named 99999
    with pytest.warns(None) as e_info:
        huduser.hud_cw(type = 1, query = "99999",
                        year = ["2010", "2011"],
                        quarter = ["1", "1", "2", "3", "4"])
...
def test_not_convert_int():
    ...
    with pytest.raises(ValueError) as e_info:
        huduser.hud_cw(type = "dwqji", query = "22031",
                        year = ["2010", "2011"],
                        quarter = ["1", "2", "3", "4"])
    with pytest.raises(ValueError) as e_info:
        huduser.hud_cw(type = "7", query = "22031ada",
                        year = ["2010", "2011"],
                        quarter = ["1", "2", "3", "4"])
    with pytest.raises(ValueError) as e_info:
        huduser.hud_cw(type = "7", query = "22031",
