...
def test_chas_state():
    ...
    test = hudchas.hud_chas_state(state = "VA")
    assert len(test) == 1
    
    test = hudchas.hud_chas_state(state = "California")
    assert len(test) == 1
    
    test = hudchas.hud_chas_state(state = "46")
    assert len(test) == 1
    
    test = hudchas.hud_chas_state(state = "Ca")
    assert len(test) == 1
...
def test_chas_county():
    test = hudchas.hud_chas_county(county = "06105")
    assert len(test) == 1
    # Query counties that have multiple...
    # Need to deal with cases when leading zero might get truncated.

    test = hudchas.hud_chas_county(county = [6105, 6115])
    assert len(test) == 2

    test = hudchas.hud_chas_county(county = ["06105", "06115"])
    assert len(test) == 2
